...
class Pages extends Admin_Controller
{
    ...
    function form($id = false)
    {
        ...
        if ($id) {
            ...
            $data['en_title'] = $page->en_title;
            $data['en_menu_title'] = $page->en_menu_title;

            $data['sequence'] = $page->sequence;
            $data['content'] = $page->content;
            $data['en_content'] = $page->en_content;
        ...
        if ($this->form_validation->run() == false) {
            ...
            $save['en_title'] = !empty($en_title) ? $en_title : $this->input->post('title');
            $en_menu_title = $this->input->post('en_menu_title');
            $save['en_menu_title'] = !empty($en_menu_title) ? $en_menu_title : $this->input->post('en_menu_title');
            ...
            $save['en_content'] = !empty($en_content) ? $en_content : $this->input->post('content');
    ...
    function link_form($id = false)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $save = array();
            $save['id'] = $id;
            $save['category_id'] = $this->input->post('cate');
            if ($save['category_id'] != '0') {
                $cate = $this->Category_model->get_category($save['category_id']);
                $save['title'] = $cate->name;
                $save['en_title'] = $cate->en_name;
                $save['en_menu_title'] = $cate->name;
                $save['menu_title'] = $cate->en_name;
                $save['url'] = $cate->slug;
            } else {
                $save['title'] = $this->input->post('title');
                $en_title = $this->input->post('en_title');
                $save['en_title'] = !empty($en_title) ? $en_title : $this->input->post('title');
                $save['en_menu_title'] = !empty($en_title) ? $en_title : $this->input->post('title');
                $save['menu_title'] = $this->input->post('title');
